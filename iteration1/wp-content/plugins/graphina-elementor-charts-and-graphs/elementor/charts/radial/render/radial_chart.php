...
if(isRestrictedAccess('radial',$this->get_id(),$settings,true)) {
    if($settings['iq_radial_restriction_content_type'] ==='password'){
        return true;
    }
    echo html_entity_decode($settings['iq_radial_restriction_content_template']);
    return true;
}
?>
...
<div class="<?php echo $settings['iq_radial_chart_card_show'] === 'yes' ? 'chart-card' : ''; ?>">
    ...
    <div class="<?php echo $settings['iq_radial_chart_border_show'] === 'yes' ? 'chart-box' : ''; ?>">
        <div class="chart-texture radial-chart-<?php esc_attr_e($this->get_id()); ?>"></div>
    </div>
</div>