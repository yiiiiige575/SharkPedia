...
class Mappress {
	...
	function __construct()  {
		...
		if (self::$options->deregister || isset($_REQUEST['mp_compat'])) {
	...
	static function block_categories_all($categories, $context) {
		if (isset($context->post) && $context->post && !in_array($context->post->post_type, self::$options->postTypes))
			return $categories;
	...
	*/
	static function get_mashup($atts) {
		...
		if (empty($mashup->query)) {
			// Round up postids.  Note that posts can be empty for GT widgets screen
			$postids = ($wp_query->get('fields') == 'ids') ? $wp_query->posts : array_map(function($post) { return $post->ID; }, $wp_query->posts);
			$mashup->pois = Mappress_Query::get_pois($postids);
		}
	...
	*/
	...
	static function template_redirect() {
		...
		// Inline poi list isn't allowed, convert to left
		$map->layout = 'left';
		$map->embed = true;
