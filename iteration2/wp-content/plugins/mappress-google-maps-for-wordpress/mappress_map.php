...
class Mappress_Map extends Mappress_Obj {
	...
	*/
	function display($atts = null, $in_iframe = false) {
		...
		if (!$in_iframe && (Mappress::$options->deregister || isset($_REQUEST['mp_compat']))) {
			$iframe = $this->display_iframe();
			if ($iframe)
				return $iframe;
		}
	...
	function display_iframe() {
		if ($this->pois) {
			return null;
			//$key = md5($data);
			//set_transient("mapp-data-$key", $this->pois, 200);
		}

		$size = Mappress::$options->size;
		$default_size = Mappress::$options->sizes[$size] ? Mappress::$options->sizes[$size] : Mappress::$options->sizes[0];

		$width = ($this->width) ? $this->width : $default_size['width'];
		$height = ($this->height) ? $this->height : $default_size['height'];
		$wrapper = (stristr($height, '%')) ? "padding-bottom: $height; height: auto" : "padding-bottom: 0; height: {$height}px";
		$atts = (array) $this;

		// 'name' is a reserved word and causes 404 errors
		unset($atts['name']);

		$atts['mapp_iframe'] = true;
		$url = get_home_url() . '?' . http_build_query($atts);
		return "<div style='width: $width'><div class='mapp-iframe-wrapper' style='$wrapper'><iframe class='mapp-iframe' src='$url'></iframe></div></div>";
	}
